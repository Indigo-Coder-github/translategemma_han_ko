...
### 파이프라인 실행 결과 (2026-02-23, aa~zc)

**Step 1: prepare_pairs.py**

| 항목 | 수치 |
|------|------|
| 입력 (articles_with_korean.jsonl) | 413,604건 |
| 번역 있음 | 412,704건 |
| 번역 없음 (제거) | 900건 |
| 중복 제거 | 36,634건 |
| **순수 출력** | **376,070건** |
| variant: clean | 203,188 (54.0%) |
| variant: annotated | 104,077 (27.7%) |
| variant: mixed | 68,805 (18.3%) |
...
|-----------|------|------|
| trailing space | 75 | |
| 한자 불일치 | 9,929 | 번역의 `(漢字)`가 원문에 미포함 |
| 중복 용어 | 22,432 | 동일 인물/용어 반복 언급 (대부분 정상) |
| 인라인 각주 | 18,070 | `content` 폴백 기사에서 주로 발생 |
| 총 플래그 기사 | 40,165 (10.7%) | |

**Step 2: align_and_chunk.py**

| 항목 | 수치 |
|------|------|
| 그대로 통과 (≤2048 tokens) | 363,339건 (71.9%) |
| 청킹 대상 (>2048 tokens) | 12,731건 |
| 생성된 청크 수 | 142,190개 (평균 11.2청크/기사) |
| **총 출력** | **505,529건** |
...
|-------|------|------|------------|-------|-----------|-------|
| train | 374,673 | 80.0% | 407 | 198,400 (53.0%) | 106,516 (28.4%) | 69,757 (18.6%) |
| val | 46,834 | 10.0% | 409 | 24,906 (53.2%) | 13,300 (28.4%) | 8,628 (18.4%) |
| test | 46,835 | 10.0% | 407 | 24,734 (52.8%) | 13,291 (28.4%) | 8,810 (18.8%) |

> 수집 미완료 데이터 추가 시 재실행 필요.
